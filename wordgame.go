...
func (*Wordgame) PostGuess(ctx *jas.Context) {
	...
	*/
	...
	nextchar := ctx.RequireString("nextchar")
	...
	// See if the guess is correct; if so, set Correct to true, return the final word, and delete the game from memory
	if len(guess) != 0 {
		if guess == game.Word {
			result.CurrentString = game.Word
			result.Correct = true
			delete(games, id)
			ctx.Data = result
			return
